...
const Dashboard = (props) => {
    ...
    useEffect(() => {
        if (isValidElement(props.matchedProfiles)) {
            let userIds = props.matchedProfiles.map(element => element.userId);
            const data = { matchedUserIds: userIds };
    ...
    return (
        <Container fluid>
            ...
            <div style={styles.sectionDivider}>
                <p style={styles.sectionDividerText}>Profiles That Matches With Yours</p>
            </div>
            <div style={styles.profileCardDiv}>
                {isValidElement(props.matchedProfiles) && props.matchedProfiles.map(user => (
                    <UserMatch key={user.userId} userDetails={user} />
                ))}
            </div>
            ...
            {isValidElement(props.matchedEvents) && props.matchedEvents.length >= slidesToShow ? (
                <Slider {...settings}>
                    {props.matchedEvents.map(event => (
                        <Event key={event.id} eventDetails={event} isSlider={true} />
                    ))}
                </Slider>
            ) : (
                <div style={styles.sliderDiv}>
                    {isValidElement(props.matchedEvents) && props.matchedEvents.map(event => (
                        <Event key={event.id} eventDetails={event} isSlider={false} />
                    ))}
                </div>
            )}
            ...
            {isValidElement(props.userEvents) && props.userEvents.length >= slidesToShow ? (
                <Slider {...settings}>
            ...
            {isValidElement(props.joinedEvents) && props.joinedEvents.length >= slidesToShow ? (
                <Slider {...settings}>
